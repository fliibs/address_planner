...
def test_sw_no_read_apb():
    ...
    reg_bank_C = RegSpace(name='reg_bank_C',size=1*KB,description='reg_bank_B,contain many regs.', external_interface='apb')
    ...
    reg_B.add(ExternalField(name='field3',bit=3,sw_access=WriteOnly,hw_access=ReadWrite,description='fied0,ext write only.'),offset=4)
    reg_B.add(ExternalField(name='field4',bit=4,sw_access=WriteOnly,hw_access=ReadWrite,description='fied0,ext write only.'),offset=8)
