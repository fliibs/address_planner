...
reg_B = Register(name='internal_reg',bit=32, description='reg0',reg_type=Normal, parity=True)

reg_B.add(Field(name='field0',bit=1,sw_access=Null, hw_access=ReadOnly,init_value=0,description='fied0, software read only.' ),offset=0)
...
reg_B.add(Field(name='field3',bit=3,sw_access=ReadWrite,hw_access=Write1Clean, init_value=0,description='fied3, hardware read only.' ),offset=6)

print(len(reg_B.parity_sw_type_list))
print(reg_B.parity_hw_type_list)
reg_C = Register(name='external_reg',description='reg1',reg_type=Normal, parity=True)
