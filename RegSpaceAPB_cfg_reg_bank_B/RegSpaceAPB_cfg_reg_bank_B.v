//==========================================================================================================================
//Key is used to ensure the consistency of file version and content, and cannot be modified.
//Version Control is the version number written when the file is generated and cannot be modified.
//ToolMessage is used to record related information of any tool that has processed the file and cannot be manually modified.
//UserMessage is used by the user to write any information, which can be modified in any way.
//Content is the actual payload of the file.
//Parameter is the additional parameter information carried by the file and cannot be modified in any way.

//Key is generated by the hash of Version Control, Content and Parameter to ensure the consistency of the file.
//These three parts do not allow any individual modification
//==========================================================================================================================


//[UHDL]Key Start [md5:f667172a26e9c7a3acb3302f08af2fd8]
//Version Control Hash: 3accddf64b1dd03abeb9b0b3e5a7ba44
//Content Hash: eebb17fd0913815a0f4c479883c9c9f3
//Parameter Hash: d41d8cd98f00b204e9800998ecf8427e
//[UHDL]Key End [md5:f667172a26e9c7a3acb3302f08af2fd8]

//[UHDL]Version Control Start [md5:3accddf64b1dd03abeb9b0b3e5a7ba44]
//[UHDL]Version Control Version:1.0.1
//[UHDL]Version Control End [md5:3accddf64b1dd03abeb9b0b3e5a7ba44]

//[UHDL]Tool Message Start [md5:be10110a876d32a488916f6dd7a39742]
//Written by UHDL in 2023-07-17 00:33:45
//[UHDL]Tool Message End [md5:be10110a876d32a488916f6dd7a39742]

//[UHDL]User Message Start [md5:d41d8cd98f00b204e9800998ecf8427e]

//[UHDL]User Message End [md5:d41d8cd98f00b204e9800998ecf8427e]

//[UHDL]Content Start [md5:eebb17fd0913815a0f4c479883c9c9f3]
module RegSpaceAPB_cfg_reg_bank_B (
	input             clk                 ,
	input             rst_n               ,
	input      [31:0] p_addr              ,
	input      [2:0]  p_prot              ,
	input             p_sel               ,
	input             p_enable            ,
	input             p_write             ,
	input      [31:0] p_wdata             ,
	input      [3:0]  p_strb              ,
	output            p_ready             ,
	output     [31:0] p_rdata             ,
	output            p_slverr            ,
	output            rs_reg0_field3_rdat ,
	output            rs_reg0_field3_rvld ,
	input             rs_reg0_field3_rrdy ,
	input             rs_reg0_field4_wdat ,
	input             rs_reg0_field4_wvld ,
	output            rs_reg0_field4_wrdy ,
	input             rs_reg0_field5_wdat ,
	input             rs_reg0_field5_wvld ,
	output            rs_reg0_field5_wrdy ,
	output            rs_reg0_field5_rdat ,
	output            rs_reg0_field5_rvld ,
	input             rs_reg0_field5_rrdy ,
	output            rs_reg1_field3_rdat ,
	output            rs_reg1_field3_rvld ,
	input             rs_reg1_field3_rrdy ,
	input             rs_reg1_field4_wdat ,
	input             rs_reg1_field4_wvld ,
	output            rs_reg1_field4_wrdy ,
	input             rs_reg1_field5_wdat ,
	input             rs_reg1_field5_wvld ,
	output            rs_reg1_field5_wrdy ,
	output            rs_reg1_field5_rdat ,
	output            rs_reg1_field5_rvld ,
	input             rs_reg1_field5_rrdy ,
	output reg [31:0] reg0_field0_p_addr  ,
	output     [2:0]  reg0_field0_p_prot  ,
	output reg        reg0_field0_p_sel   ,
	output reg        reg0_field0_p_enable,
	output            reg0_field0_p_write ,
	output     [31:0] reg0_field0_p_wdata ,
	output     [3:0]  reg0_field0_p_strb  ,
	input             reg0_field0_p_ready ,
	input      [31:0] reg0_field0_p_rdata ,
	input             reg0_field0_p_slverr,
	output reg [31:0] reg0_field1_p_addr  ,
	output     [2:0]  reg0_field1_p_prot  ,
	output reg        reg0_field1_p_sel   ,
	output reg        reg0_field1_p_enable,
	output            reg0_field1_p_write ,
	output     [31:0] reg0_field1_p_wdata ,
	output     [3:0]  reg0_field1_p_strb  ,
	input             reg0_field1_p_ready ,
	input      [31:0] reg0_field1_p_rdata ,
	input             reg0_field1_p_slverr,
	output reg [31:0] reg0_field2_p_addr  ,
	output     [2:0]  reg0_field2_p_prot  ,
	output reg        reg0_field2_p_sel   ,
	output reg        reg0_field2_p_enable,
	output            reg0_field2_p_write ,
	output     [31:0] reg0_field2_p_wdata ,
	output     [3:0]  reg0_field2_p_strb  ,
	input             reg0_field2_p_ready ,
	input      [31:0] reg0_field2_p_rdata ,
	input             reg0_field2_p_slverr,
	output reg [31:0] reg0_field6_p_addr  ,
	output     [2:0]  reg0_field6_p_prot  ,
	output reg        reg0_field6_p_sel   ,
	output reg        reg0_field6_p_enable,
	output            reg0_field6_p_write ,
	output     [31:0] reg0_field6_p_wdata ,
	output     [3:0]  reg0_field6_p_strb  ,
	input             reg0_field6_p_ready ,
	input      [31:0] reg0_field6_p_rdata ,
	input             reg0_field6_p_slverr,
	output reg [31:0] reg1_field0_p_addr  ,
	output     [2:0]  reg1_field0_p_prot  ,
	output reg        reg1_field0_p_sel   ,
	output reg        reg1_field0_p_enable,
	output            reg1_field0_p_write ,
	output     [31:0] reg1_field0_p_wdata ,
	output     [3:0]  reg1_field0_p_strb  ,
	input             reg1_field0_p_ready ,
	input      [31:0] reg1_field0_p_rdata ,
	input             reg1_field0_p_slverr,
	output reg [31:0] reg1_field1_p_addr  ,
	output     [2:0]  reg1_field1_p_prot  ,
	output reg        reg1_field1_p_sel   ,
	output reg        reg1_field1_p_enable,
	output            reg1_field1_p_write ,
	output     [31:0] reg1_field1_p_wdata ,
	output     [3:0]  reg1_field1_p_strb  ,
	input             reg1_field1_p_ready ,
	input      [31:0] reg1_field1_p_rdata ,
	input             reg1_field1_p_slverr,
	output reg [31:0] reg1_field2_p_addr  ,
	output     [2:0]  reg1_field2_p_prot  ,
	output reg        reg1_field2_p_sel   ,
	output reg        reg1_field2_p_enable,
	output            reg1_field2_p_write ,
	output     [31:0] reg1_field2_p_wdata ,
	output     [3:0]  reg1_field2_p_strb  ,
	input             reg1_field2_p_ready ,
	input      [31:0] reg1_field2_p_rdata ,
	input             reg1_field2_p_slverr,
	output reg [31:0] reg1_field6_p_addr  ,
	output     [2:0]  reg1_field6_p_prot  ,
	output reg        reg1_field6_p_sel   ,
	output reg        reg1_field6_p_enable,
	output            reg1_field6_p_write ,
	output     [31:0] reg1_field6_p_wdata ,
	output     [3:0]  reg1_field6_p_strb  ,
	input             reg1_field6_p_ready ,
	input      [31:0] reg1_field6_p_rdata ,
	input             reg1_field6_p_slverr);

	//Wire define for this module.
	reg  [0:0]  p_ready_r;
	wire [0:0]  p_rready ;
	wire [0:0]  p_wready ;
	reg  [31:0] p_rdata_r;

	//Wire define for sub module.
	wire        rs_rreq_vld           ;
	wire        rs_rreq_rdy           ;
	wire [31:0] rs_rack_data          ;
	wire        rs_rack_vld           ;
	wire        rs_rack_rdy           ;
	wire [31:0] rs_wreq_data          ;
	wire        rs_wreq_vld           ;
	wire        rs_wreq_rdy           ;
	wire        rs_reg0_sw_field0_rdat;
	wire        rs_reg0_sw_field0_rvld;
	wire        rs_reg0_sw_field0_rrdy;
	wire        rs_reg0_sw_field0_wdat;
	wire        rs_reg0_sw_field0_wvld;
	wire        rs_reg0_sw_field0_wrdy;
	wire        rs_reg0_sw_field1_rdat;
	wire        rs_reg0_sw_field1_rvld;
	wire        rs_reg0_sw_field1_rrdy;
	wire        rs_reg0_sw_field1_wdat;
	wire        rs_reg0_sw_field1_wvld;
	wire        rs_reg0_sw_field1_wrdy;
	wire        rs_reg0_sw_field2_rdat;
	wire        rs_reg0_sw_field2_rvld;
	wire        rs_reg0_sw_field2_rrdy;
	wire        rs_reg0_sw_field2_wdat;
	wire        rs_reg0_sw_field2_wvld;
	wire        rs_reg0_sw_field2_wrdy;
	wire [1:0]  rs_reg0_sw_field6_rdat;
	wire        rs_reg0_sw_field6_rvld;
	wire        rs_reg0_sw_field6_rrdy;
	wire [1:0]  rs_reg0_sw_field6_wdat;
	wire        rs_reg0_sw_field6_wvld;
	wire        rs_reg0_sw_field6_wrdy;
	wire        rs_reg1_sw_field0_rdat;
	wire        rs_reg1_sw_field0_rvld;
	wire        rs_reg1_sw_field0_rrdy;
	wire        rs_reg1_sw_field0_wdat;
	wire        rs_reg1_sw_field0_wvld;
	wire        rs_reg1_sw_field0_wrdy;
	wire        rs_reg1_sw_field1_rdat;
	wire        rs_reg1_sw_field1_rvld;
	wire        rs_reg1_sw_field1_rrdy;
	wire        rs_reg1_sw_field1_wdat;
	wire        rs_reg1_sw_field1_wvld;
	wire        rs_reg1_sw_field1_wrdy;
	wire        rs_reg1_sw_field2_rdat;
	wire        rs_reg1_sw_field2_rvld;
	wire        rs_reg1_sw_field2_rrdy;
	wire        rs_reg1_sw_field2_wdat;
	wire        rs_reg1_sw_field2_wvld;
	wire        rs_reg1_sw_field2_wrdy;
	wire [1:0]  rs_reg1_sw_field6_rdat;
	wire        rs_reg1_sw_field6_rvld;
	wire        rs_reg1_sw_field6_rrdy;
	wire [1:0]  rs_reg1_sw_field6_wdat;
	wire        rs_reg1_sw_field6_wvld;
	wire        rs_reg1_sw_field6_wrdy;

	//Wire define for Inout.

	//Wire sub module connect to this module and inter module connect.
	assign p_ready = p_ready_r;
	
	assign p_rdata = p_rdata_r;
	
	assign p_slverr = 1'b0;
	
	always @(posedge clk or negedge rst_n) begin
	    if(~rst_n) p_ready_r <= 1'b0;
	    else begin
	        if((p_rready || p_wready)) p_ready_r <= 1'b1;
	        else p_ready_r <= 1'b0;
	    end
	end
	
	assign p_rready = (rreq_vld && rs_rreq_rdy);
	
	assign p_wready = rs_wreq_rdy;
	
	always @(posedge clk or negedge rst_n) begin
	    if(~rst_n) p_rdata_r <= 32'b0;
	    else begin
	        if((rreq_vld && rs_rreq_rdy)) p_rdata_r <= rs_rack_data;
	        else p_rdata_r <= 32'b0;
	    end
	end
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field0_p_addr = p_addr;
	    else reg0_field0_p_addr = 32'b0;
	end
	
	assign reg0_field0_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field0_p_sel = p_sel;
	    else reg0_field0_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field0_p_enable = p_enable;
	    else reg0_field0_p_enable = 1'b0;
	end
	
	assign reg0_field0_p_write = p_write;
	
	assign reg0_field0_p_wdata = p_wdata;
	
	assign reg0_field0_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field1_p_addr = p_addr;
	    else reg0_field1_p_addr = 32'b0;
	end
	
	assign reg0_field1_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field1_p_sel = p_sel;
	    else reg0_field1_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field1_p_enable = p_enable;
	    else reg0_field1_p_enable = 1'b0;
	end
	
	assign reg0_field1_p_write = p_write;
	
	assign reg0_field1_p_wdata = p_wdata;
	
	assign reg0_field1_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field2_p_addr = p_addr;
	    else reg0_field2_p_addr = 32'b0;
	end
	
	assign reg0_field2_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field2_p_sel = p_sel;
	    else reg0_field2_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field2_p_enable = p_enable;
	    else reg0_field2_p_enable = 1'b0;
	end
	
	assign reg0_field2_p_write = p_write;
	
	assign reg0_field2_p_wdata = p_wdata;
	
	assign reg0_field2_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field6_p_addr = p_addr;
	    else reg0_field6_p_addr = 32'b0;
	end
	
	assign reg0_field6_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field6_p_sel = p_sel;
	    else reg0_field6_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b0)) reg0_field6_p_enable = p_enable;
	    else reg0_field6_p_enable = 1'b0;
	end
	
	assign reg0_field6_p_write = p_write;
	
	assign reg0_field6_p_wdata = p_wdata;
	
	assign reg0_field6_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field0_p_addr = p_addr;
	    else reg1_field0_p_addr = 32'b0;
	end
	
	assign reg1_field0_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field0_p_sel = p_sel;
	    else reg1_field0_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field0_p_enable = p_enable;
	    else reg1_field0_p_enable = 1'b0;
	end
	
	assign reg1_field0_p_write = p_write;
	
	assign reg1_field0_p_wdata = p_wdata;
	
	assign reg1_field0_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field1_p_addr = p_addr;
	    else reg1_field1_p_addr = 32'b0;
	end
	
	assign reg1_field1_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field1_p_sel = p_sel;
	    else reg1_field1_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field1_p_enable = p_enable;
	    else reg1_field1_p_enable = 1'b0;
	end
	
	assign reg1_field1_p_write = p_write;
	
	assign reg1_field1_p_wdata = p_wdata;
	
	assign reg1_field1_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field2_p_addr = p_addr;
	    else reg1_field2_p_addr = 32'b0;
	end
	
	assign reg1_field2_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field2_p_sel = p_sel;
	    else reg1_field2_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field2_p_enable = p_enable;
	    else reg1_field2_p_enable = 1'b0;
	end
	
	assign reg1_field2_p_write = p_write;
	
	assign reg1_field2_p_wdata = p_wdata;
	
	assign reg1_field2_p_strb = p_strb;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field6_p_addr = p_addr;
	    else reg1_field6_p_addr = 32'b0;
	end
	
	assign reg1_field6_p_prot = 3'b0;
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field6_p_sel = p_sel;
	    else reg1_field6_p_sel = 1'b0;
	end
	
	always @(*) begin
	    if((p_addr == 32'b1)) reg1_field6_p_enable = p_enable;
	    else reg1_field6_p_enable = 1'b0;
	end
	
	assign reg1_field6_p_write = p_write;
	
	assign reg1_field6_p_wdata = p_wdata;
	
	assign reg1_field6_p_strb = p_strb;
	

	//Wire this module connect to sub module.
	assign rs_clk = clk;
	
	assign rs_rst_n = rst_n;
	
	assign rs_rreq_addr = p_addr;
	
	assign rs_rreq_vld = ((!p_write) && p_sel);
	
	assign rs_rack_rdy = ((!p_write) && p_sel && p_enable);
	
	assign rs_wreq_addr = p_addr;
	
	assign rs_wreq_data = {(p_wdata[7:0] & {p_strb[0:0], p_strb[0:0], p_strb[0:0], p_strb[0:0], p_strb[0:0], p_strb[0:0], p_strb[0:0], p_strb[0:0]}), (p_wdata[15:8] & {p_strb[1:1], p_strb[1:1], p_strb[1:1], p_strb[1:1], p_strb[1:1], p_strb[1:1], p_strb[1:1], p_strb[1:1]}), (p_wdata[23:16] & {p_strb[2:2], p_strb[2:2], p_strb[2:2], p_strb[2:2], p_strb[2:2], p_strb[2:2], p_strb[2:2], p_strb[2:2]}), (p_wdata[31:24] & {p_strb[3:3], p_strb[3:3], p_strb[3:3], p_strb[3:3], p_strb[3:3], p_strb[3:3], p_strb[3:3], p_strb[3:3]})};
	
	assign rs_wreq_vld = (p_write && p_sel && p_enable);
	
	assign rs_reg0_sw_field0_rdat = reg0_field0_p_rdata[0:0];
	
	assign rs_reg0_sw_field0_rrdy = (reg0_field0_p_ready && (!u_reg0_field0_p_write));
	
	assign rs_reg0_sw_field0_wrdy = (reg0_field0_p_ready && u_reg0_field0_p_write);
	
	assign rs_reg0_sw_field1_rdat = reg0_field1_p_rdata[3:3];
	
	assign rs_reg0_sw_field1_rrdy = (reg0_field1_p_ready && (!u_reg0_field1_p_write));
	
	assign rs_reg0_sw_field1_wrdy = (reg0_field1_p_ready && u_reg0_field1_p_write);
	
	assign rs_reg0_sw_field2_rdat = reg0_field2_p_rdata[5:5];
	
	assign rs_reg0_sw_field2_rrdy = (reg0_field2_p_ready && (!u_reg0_field2_p_write));
	
	assign rs_reg0_sw_field2_wrdy = (reg0_field2_p_ready && u_reg0_field2_p_write);
	
	assign rs_reg0_sw_field6_rdat = reg0_field6_p_rdata[14:13];
	
	assign rs_reg0_sw_field6_rrdy = (reg0_field6_p_ready && (!u_reg0_field6_p_write));
	
	assign rs_reg0_sw_field6_wrdy = (reg0_field6_p_ready && u_reg0_field6_p_write);
	
	assign rs_reg1_sw_field0_rdat = reg1_field0_p_rdata[0:0];
	
	assign rs_reg1_sw_field0_rrdy = (reg1_field0_p_ready && (!u_reg1_field0_p_write));
	
	assign rs_reg1_sw_field0_wrdy = (reg1_field0_p_ready && u_reg1_field0_p_write);
	
	assign rs_reg1_sw_field1_rdat = reg1_field1_p_rdata[3:3];
	
	assign rs_reg1_sw_field1_rrdy = (reg1_field1_p_ready && (!u_reg1_field1_p_write));
	
	assign rs_reg1_sw_field1_wrdy = (reg1_field1_p_ready && u_reg1_field1_p_write);
	
	assign rs_reg1_sw_field2_rdat = reg1_field2_p_rdata[5:5];
	
	assign rs_reg1_sw_field2_rrdy = (reg1_field2_p_ready && (!u_reg1_field2_p_write));
	
	assign rs_reg1_sw_field2_wrdy = (reg1_field2_p_ready && u_reg1_field2_p_write);
	
	assign rs_reg1_sw_field6_rdat = reg1_field6_p_rdata[14:13];
	
	assign rs_reg1_sw_field6_rrdy = (reg1_field6_p_ready && (!u_reg1_field6_p_write));
	
	assign rs_reg1_sw_field6_wrdy = (reg1_field6_p_ready && u_reg1_field6_p_write);
	

	//module inst.
	RegSpaceBase_cfg_reg_bank_B rs (
		.clk(rs_clk),
		.rst_n(rs_rst_n),
		.rreq_addr(rs_rreq_addr),
		.rreq_vld(rs_rreq_vld),
		.rreq_rdy(rs_rreq_rdy),
		.rack_data(rs_rack_data),
		.rack_vld(rs_rack_vld),
		.rack_rdy(rs_rack_rdy),
		.wreq_addr(rs_wreq_addr),
		.wreq_data(rs_wreq_data),
		.wreq_vld(rs_wreq_vld),
		.wreq_rdy(rs_wreq_rdy),
		.reg0_sw_field0_rdat(rs_reg0_sw_field0_rdat),
		.reg0_sw_field0_rvld(rs_reg0_sw_field0_rvld),
		.reg0_sw_field0_rrdy(rs_reg0_sw_field0_rrdy),
		.reg0_sw_field0_wdat(rs_reg0_sw_field0_wdat),
		.reg0_sw_field0_wvld(rs_reg0_sw_field0_wvld),
		.reg0_sw_field0_wrdy(rs_reg0_sw_field0_wrdy),
		.reg0_sw_field1_rdat(rs_reg0_sw_field1_rdat),
		.reg0_sw_field1_rvld(rs_reg0_sw_field1_rvld),
		.reg0_sw_field1_rrdy(rs_reg0_sw_field1_rrdy),
		.reg0_sw_field1_wdat(rs_reg0_sw_field1_wdat),
		.reg0_sw_field1_wvld(rs_reg0_sw_field1_wvld),
		.reg0_sw_field1_wrdy(rs_reg0_sw_field1_wrdy),
		.reg0_sw_field2_rdat(rs_reg0_sw_field2_rdat),
		.reg0_sw_field2_rvld(rs_reg0_sw_field2_rvld),
		.reg0_sw_field2_rrdy(rs_reg0_sw_field2_rrdy),
		.reg0_sw_field2_wdat(rs_reg0_sw_field2_wdat),
		.reg0_sw_field2_wvld(rs_reg0_sw_field2_wvld),
		.reg0_sw_field2_wrdy(rs_reg0_sw_field2_wrdy),
		.reg0_field3_rdat(rs_reg0_field3_rdat),
		.reg0_field3_rvld(rs_reg0_field3_rvld),
		.reg0_field3_rrdy(rs_reg0_field3_rrdy),
		.reg0_field4_wdat(rs_reg0_field4_wdat),
		.reg0_field4_wvld(rs_reg0_field4_wvld),
		.reg0_field4_wrdy(rs_reg0_field4_wrdy),
		.reg0_field5_wdat(rs_reg0_field5_wdat),
		.reg0_field5_wvld(rs_reg0_field5_wvld),
		.reg0_field5_wrdy(rs_reg0_field5_wrdy),
		.reg0_field5_rdat(rs_reg0_field5_rdat),
		.reg0_field5_rvld(rs_reg0_field5_rvld),
		.reg0_field5_rrdy(rs_reg0_field5_rrdy),
		.reg0_sw_field6_rdat(rs_reg0_sw_field6_rdat),
		.reg0_sw_field6_rvld(rs_reg0_sw_field6_rvld),
		.reg0_sw_field6_rrdy(rs_reg0_sw_field6_rrdy),
		.reg0_sw_field6_wdat(rs_reg0_sw_field6_wdat),
		.reg0_sw_field6_wvld(rs_reg0_sw_field6_wvld),
		.reg0_sw_field6_wrdy(rs_reg0_sw_field6_wrdy),
		.reg1_sw_field0_rdat(rs_reg1_sw_field0_rdat),
		.reg1_sw_field0_rvld(rs_reg1_sw_field0_rvld),
		.reg1_sw_field0_rrdy(rs_reg1_sw_field0_rrdy),
		.reg1_sw_field0_wdat(rs_reg1_sw_field0_wdat),
		.reg1_sw_field0_wvld(rs_reg1_sw_field0_wvld),
		.reg1_sw_field0_wrdy(rs_reg1_sw_field0_wrdy),
		.reg1_sw_field1_rdat(rs_reg1_sw_field1_rdat),
		.reg1_sw_field1_rvld(rs_reg1_sw_field1_rvld),
		.reg1_sw_field1_rrdy(rs_reg1_sw_field1_rrdy),
		.reg1_sw_field1_wdat(rs_reg1_sw_field1_wdat),
		.reg1_sw_field1_wvld(rs_reg1_sw_field1_wvld),
		.reg1_sw_field1_wrdy(rs_reg1_sw_field1_wrdy),
		.reg1_sw_field2_rdat(rs_reg1_sw_field2_rdat),
		.reg1_sw_field2_rvld(rs_reg1_sw_field2_rvld),
		.reg1_sw_field2_rrdy(rs_reg1_sw_field2_rrdy),
		.reg1_sw_field2_wdat(rs_reg1_sw_field2_wdat),
		.reg1_sw_field2_wvld(rs_reg1_sw_field2_wvld),
		.reg1_sw_field2_wrdy(rs_reg1_sw_field2_wrdy),
		.reg1_field3_rdat(rs_reg1_field3_rdat),
		.reg1_field3_rvld(rs_reg1_field3_rvld),
		.reg1_field3_rrdy(rs_reg1_field3_rrdy),
		.reg1_field4_wdat(rs_reg1_field4_wdat),
		.reg1_field4_wvld(rs_reg1_field4_wvld),
		.reg1_field4_wrdy(rs_reg1_field4_wrdy),
		.reg1_field5_wdat(rs_reg1_field5_wdat),
		.reg1_field5_wvld(rs_reg1_field5_wvld),
		.reg1_field5_wrdy(rs_reg1_field5_wrdy),
		.reg1_field5_rdat(rs_reg1_field5_rdat),
		.reg1_field5_rvld(rs_reg1_field5_rvld),
		.reg1_field5_rrdy(rs_reg1_field5_rrdy),
		.reg1_sw_field6_rdat(rs_reg1_sw_field6_rdat),
		.reg1_sw_field6_rvld(rs_reg1_sw_field6_rvld),
		.reg1_sw_field6_rrdy(rs_reg1_sw_field6_rrdy),
		.reg1_sw_field6_wdat(rs_reg1_sw_field6_wdat),
		.reg1_sw_field6_wvld(rs_reg1_sw_field6_wvld),
		.reg1_sw_field6_wrdy(rs_reg1_sw_field6_wrdy));

endmodule
//[UHDL]Content End [md5:eebb17fd0913815a0f4c479883c9c9f3]

//[UHDL]Parameter Start [md5:d41d8cd98f00b204e9800998ecf8427e]

//[UHDL]Parameter End [md5:d41d8cd98f00b204e9800998ecf8427e]

