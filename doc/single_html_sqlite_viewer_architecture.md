...
- `Bank Info` 和 `Reg Info` 标题栏各自提供统一的 28 px 可视列按钮，弹出菜单可连续
  ...
  两侧按钮以中性状态点独立提示当前存在隐藏列。默认列宽重新按内容预算分配：Bank 为
  Name 176、Start 92、End 92、Size 76、Description 260 px，Reg 为 Name 132、Position 76、
  External 64、Software/Hardware Access 各 102、Description 248 px；End 和 External 仍默认
  隐藏。所有列仍可拖动，最小列宽由 60 px 降至 32 px；缩窄时所有表格单元格统一直接
  裁切，不显示省略号、不换行，也不覆盖相邻单元格。每次列显隐后，最后一个可见列作为
  流动列填满剩余空间；此前各列严格保持用户设置的像素宽度，不再随面板整体宽度按比例
  伸缩。流动列仍保留首选最小宽度，面板不足时继续触发横向滚动。
