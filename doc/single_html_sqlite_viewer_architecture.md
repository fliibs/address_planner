...
- 标题栏统计数据之后、页面右上角提供统一的 28 px 明暗主题切换按钮；React 启动前的静态壳
  与挂载后的 React 标题栏保持相同的位置、尺寸和交互语义，加载完成时不会发生跳位。两套
  配色参考 VS Code 经典 `Light+` / `Dark+` 主题。亮色将上一版低亮冷蓝灰调整为适度提亮、
  降低灰感但不回到纯白的冷蓝灰层级，用不同深浅区分 workbench 画布、editor/表格内容面与
  表头/菜单抬升层；暗色仍采用 `#1e1e1e` editor 与 `#252526` workbench，主交互和选择提示
  统一使用 `#007acc` 蓝色。两套语义色令牌完整覆盖
...
- Viewer 保留默认左右等宽的 `Bank Info / Reg Info` 视觉结构，使用 VS Code 经典主题式的
  editor/workbench 明暗层级：亮色画布、内容面与抬升层采用适度提亮、降低灰感但避免纯白的
  冷蓝灰色阶，暗色内容面仍为 `#1e1e1e`、外围工作区仍为 `#252526`，并以 `#007acc` 蓝色强调
  交互状态；空表体、普通表面、层级行和抬升表头之间仍有清晰的对比关系。Register 整行均可
  选择，四个较长的
